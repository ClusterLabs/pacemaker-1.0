/* $Id: f.c,v 1.1 2004/08/03 06:32:21 deng.pan Exp $ */
/* 
 * f.c: Event Service API test case for:saEvtEventRetentionTimeClear
 *
...
int main(int argc, char **argv)
{
	...
	if(saEvtEventAttributesSet(event_hd, &pattern_array, 1, 10000, &publisher_name) !=
		SA_OK){
	...
	if(saEvtEventPublish(event_hd, event_data, data_size, &event_id) != 
		SA_OK){
		printf("Event publish fail\n");
		return 1;
	}
	
	// retention time clear
	if(saEvtEventRetentionTimeClear(channel_handle, event_id) 
		== SA_OK){
		printf("Event clear retention time(1) success\n");
	}else{
		printf("Event clear retention time(1) fail\n");
	}

	if(saEvtEventRetentionTimeClear(channel_handle, event_id) 
		== SA_ERR_NOT_EXIST){
		printf("Event clear retention time(2) success\n");
	}else{
		printf("Event clear retention time(2) fail\n");
	}
	...
	if(saEvtEventRetentionTimeClear(channel_handle, event_id) 
		== SA_ERR_BAD_HANDLE){
		printf("Event clear retention time(3) success\n");
	}else{
		printf("Event clear retention time(3) fail\n");
	}
