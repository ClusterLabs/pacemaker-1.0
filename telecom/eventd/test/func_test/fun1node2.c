/* $Id: fun1node2.c,v 1.1 2004/08/03 06:32:22 deng.pan Exp $ */
/* 
 * fun1node2.c: Funtion Test Case 1 for Event Service Test
...
int main(int argc, char **argv)
{
	...
	if(saEvtChannelOpen(evt_handle, &ch_name, 6, 1000000, &channel_handle[1])
			!= SA_OK){
		saEvtFinalize(evt_handle);
		syslog (LOG_INFO|LOG_LOCAL7, "%s \n", Fail_message) ;
		return -1;
	...
	if(saEvtChannelOpen(evt_handle, &ch_name, 3, 1000000, &channel_handle[2])
			!= SA_OK){
		syslog (LOG_INFO|LOG_LOCAL7, "%s \n", Fail_message) ;
		return -1;
	}
	...
	if(saEvtChannelOpen(evt_handle, &ch_name, 2, 1000000, &channel_handle[3])
			!= SA_OK){
		saEvtFinalize(evt_handle);
		syslog (LOG_INFO|LOG_LOCAL7, "%s \n", Fail_message) ;
		return -1;
	...
	if(saEvtChannelOpen(evt_handle, &ch_name, 1, 1000000, &channel_handle[4])
			!= SA_OK){
		saEvtFinalize(evt_handle);
		syslog (LOG_INFO|LOG_LOCAL7, "%s \n", Fail_message) ;
		return -1;
