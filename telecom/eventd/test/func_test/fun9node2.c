...
int main(int argc, char **argv)
{
	...
	for(i=0;i<4;i++){
		if(saEvtEventSubscribe(channel_handle[i], &filter_mode[0], 4*i+1)
					!= SA_OK){
			for(i=0;i<4;i++) saEvtChannelClose(channel_handle[i]);
			freemode();
			saEvtFinalize(evt_handle);
	...
	FD_SET(fd, &rset);
	tv.tv_sec=2;
	select_ret = select(100, &rset, NULL,NULL, NULL);
	if((select_ret == -1) || (select_ret == 0))
		{
			syslog (LOG_INFO|LOG_LOCAL7, "select error: %d\n",select_ret) ;
			for(i=0;i<4;i++) saEvtChannelClose(channel_handle[i]);
			freemode();
			saEvtFinalize(evt_handle);
			syslog (LOG_INFO|LOG_LOCAL7, "%s sle\n", Fail_message) ;
			return -1;
		}else{
			sleep(1);
