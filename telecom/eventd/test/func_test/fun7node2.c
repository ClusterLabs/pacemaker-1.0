/* $Id: fun7node2.c,v 1.1 2004/08/03 06:32:22 deng.pan Exp $ */
/* 
 * fun7node2.c: Funtion Test Case 7 for Event Service Test
...
#define EVENT_DATA  "Publish M:N Function Test Case event"
#define Event_Priority 1
#define Event_retentionTime 10000
#define PublishName "f7node1"
#define Pattern1 "func07"
...
int main(int argc, char **argv)
{
	...
	//Step 1
	ch_name.length = sizeof("fun07");
	memcpy(ch_name.value, "fun07", sizeof("fun07"));
	...
	//step 6: publish event 1 on node 2 channel 1&2
	//event allocate
	...
	if(saEvtEventAllocate(channel_handle2, &event_handle1)!= SA_OK)
	{
		saEvtChannelClose(channel_handle1);
		saEvtChannelClose(channel_handle2);
		saEvtFinalize(evt_handle);
	...
	if(saEvtEventAttributesSet(event_handle1, &pattern_array, Event_Priority, Event_retentionTime, &publisher_name)!= SA_OK)
	{
		saEvtChannelClose(channel_handle1);
		saEvtChannelClose(channel_handle2);
		saEvtFinalize(evt_handle);
	...
	if(saEvtEventPublish(event_handle1, event_data, data_size, &event_id)!= SA_OK)
	{
		saEvtChannelClose(channel_handle1);
		saEvtChannelClose(channel_handle2);
		saEvtFinalize(evt_handle);
	...
	if(saEvtEventFree(event_handle1)!= SA_OK)
	{
		saEvtChannelClose(channel_handle1);
		saEvtChannelClose(channel_handle2);
		saEvtFinalize(evt_handle);
	...
	if((select_ret == -1) || (select_ret == 0))
	{
		...
		if(saEvtDispatch(evt_handle, SA_DISPATCH_ALL)!= SA_OK || nCmpResult<0 || nTimes!=18)
		{
			saEvtChannelClose(channel_handle1);
			saEvtChannelClose(channel_handle2);
			saEvtFinalize(evt_handle);
