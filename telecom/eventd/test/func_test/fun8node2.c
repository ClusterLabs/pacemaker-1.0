/* $Id: fun8node2.c,v 1.1 2004/08/03 06:32:22 deng.pan Exp $ */
/* 
 * fun8node2.c: Funtion Test Case 8 for Event Service Test
...
{	
	...
	if(receivecmp(event_handle,eventDataSize,c,pattern_array, nTimes,
	...
	if((select_ret == -1) || (select_ret == 0))
		{
			...
			if(saEvtDispatch(evt_handle, SA_DISPATCH_ALL)!= SA_OK || nCmpResult<0 || nTimes!=4)
			{
				saEvtChannelClose(channel_handle1);
				saEvtFinalize(evt_handle);
				syslog (LOG_INFO|LOG_LOCAL7, "%s \n", Fail_message) ;
