/* $Id: close3node2.c,v 1.1 2004/06/30 03:44:19 deng.pan Exp $ */
/* 
 * close3node2.c: Test data checkpoint function : saCkptCheckpointClose 
 *
...
void initparam(void)
{
	...
	memcpy (ckpt_name.value, CaseName, ckpt_name.length);
...
/*
 *Description :
 *	Unlink the specified checkpoint name. Check the unlink lable. Try to 
 *open it using the same name again.
 *Returns:
 *   -1: fail
 *   0 : success
 */
...
int setunlink (void)
{
	...
	SaCkptCheckpointHandleT tmphandle = -1;
	
	if (saCkptCheckpointUnlink (&libhandle , &ckpt_name) != SA_OK )
		return -1 ;

	ckpt_error = saCkptCheckpointOpen (&tmphandle, &ckpt_name, &ckpt_create_attri, 
			SA_CKPT_CHECKPOINT_COLOCATED|SA_CKPT_CHECKPOINT_WRITE|SA_CKPT_CHECKPOINT_READ, 
...
int main(int argc, char **argv)
{
	...
	/* wait for node 1 (slave) operation*/
	syslog (LOG_INFO|LOG_LOCAL7, "ckpt_signal %d %d\n", count++, SIGUSR1) ;
	pause () ;
	
	/* Check really slave node creation effects  */
	if (setunlink () < 0)
		{
			finalize () ;
			syslog (LOG_INFO|LOG_LOCAL7, "ckpt_fail\n") ;
	...
	finalize () ;
	syslog (LOG_INFO|LOG_LOCAL7, "ckpt_signal success\n") ;
