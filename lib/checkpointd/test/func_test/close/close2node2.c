/* $Id: close2node2.c,v 1.1 2004/06/30 03:44:19 deng.pan Exp $ */
/* 
 * close2node2.c: Test data checkpoint function : saCkptCheckpointClose 
 *
...
int main(int argc, char **argv)
{
	...
	bzero (slavenode, sizeof(slavenode)) ;	
	strcpy (slavenode, argv[1]) ;
	...
	/* Delete the local replica. Slave node will be changed into active node */
	saCkptCheckpointClose (&cphandle) ;
	saCkptFinalize (&libhandle) ;
	cphandle = -1 ;
	libhandle = -1 ;
	...
	/* wait for node 1 (active) operation*/
	syslog (LOG_INFO|LOG_LOCAL7, "ckpt_signal %d %d\n", count++, SIGUSR1) ;
	pause () ;
