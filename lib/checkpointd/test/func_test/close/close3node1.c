/* $Id: close3node1.c,v 1.1 2004/06/30 03:44:19 deng.pan Exp $ */
/* 
 * close3node1.c: Test data checkpoint function : saCkptCheckpointClose 
 *
...
int inittest(void);
int opensync (void);
int checkunlink (void);
int freereplica (void);
 ...
 * parameters :
 * 		flag : 0, open with colocated flag
 * 			   1, open without colocated flag
 * returns : 
 * 		0: success
 * 		-1 : fail
 */
...
/*Description:
 * 	Check unlink lable. Try open again. Open should fail.
 *Returns
 *	-1: fail
 *	0 : success
 */
...
int checkunlink (void)
{
	SaCkptCheckpointHandleT tmphandle ;
	
	ckpt_error = 
		saCkptCheckpointOpen (&tmphandle, &ckpt_name, &ckpt_create_attri, 
			SA_CKPT_CHECKPOINT_COLOCATED|SA_CKPT_CHECKPOINT_WRITE|SA_CKPT_CHECKPOINT_READ, 
			open_timeout, &libhandle) ;

	if (ckpt_error == SA_OK)
		{
			saCkptCheckpointClose (&tmphandle) ;	
			return -1 ;
		}
...
/*Description:
 * 	Free the local replia and check status to verify.
 *
 *Returns:
 *  -1 : fail
 *  0  : success
...
int freereplica (void)
{
	if (saCkptCheckpointClose (&cphandle) != SA_OK 
			|| saCkptFinalize (&libhandle) != SA_OK )
		{
			cphandle = libhandle = -1 ;
			return -1 ;
		}
...
int main(int argc, char **argv)
{
	...
	ckpt_create_attri.retentionDuration = 1000000;
	...
	/* free local replica and check status to verify */
	if (freereplica () < 0 )
		{
			finalize ();
			syslog (LOG_INFO|LOG_LOCAL7, "ckpt_fail\n") ;
	...
	/* Active node has no open.Only local open for the active replica*/ 
	if (freereplica () < 0)
		{
			finalize ();
			syslog (LOG_INFO|LOG_LOCAL7, "ckpt_fail\n") ;
