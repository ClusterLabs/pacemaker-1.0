/* $Id: sync4node2.c,v 1.3 2004/10/09 01:49:41 lge Exp $ */
/* 
...
/*Description:
 *	Create one section in remote replica. Check local status.
 *Returns:
 *  -1 : fail 
 *  0  : success
 */
int createsect (void)
{
	...
	ckpt_error = saCkptSectionCreate (&cphandle, &sectattri ,  
									  NULL, 0) ;
	...
	for (i =0 ; i< 5 ; i++)
		{
			...
			sectwrite.dataOffset += strlen(tmp);
